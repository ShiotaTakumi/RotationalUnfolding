#!/usr/bin/env python3
"""
Drawing utility for visualizing raw.jsonl records as SVG files.

Converts each partial unfolding record in raw.jsonl to an individual SVG file
for visual inspection and verification purposes.

This is a verification utility, not a Phase output.

raw.jsonl の各レコードを SVG ファイルとして可視化するユーティリティ。
視覚的な検査と検証のために、各部分展開図を個別の SVG ファイルに変換します。

これは検証用ユーティリティであり、Phase の成果物ではありません。

Usage:
    python reorg/python/drawing/draw_raw_jsonl.py --input <raw.jsonl> --outdir <output_dir>
...
def draw_raw_jsonl(raw_jsonl_path, output_dir):
    ...
    # Compute zero-padding width (0-based indexing)
    # ゼロ埋め幅を計算（0-based インデックス）
...
def main():
    parser = argparse.ArgumentParser(
        description="Draw SVG files from raw.jsonl (verification utility)"
    )
    ...
    print(f"Drawing partial unfoldings from: {args.input}")
    print(f"Output directory: {args.outdir}")
    print("")
    
    try:
        num_generated = draw_raw_jsonl(args.input, args.outdir)
