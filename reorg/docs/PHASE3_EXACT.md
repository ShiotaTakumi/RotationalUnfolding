...
Each component is implemented as a separate Python module under `reorg/python/`. There are no import dependencies between them. Phase 3 reads files produced by earlier phases but shares no code with them.

各コンポーネントは `reorg/python/` 配下の独立した Python モジュールとして実装されます。モジュール間のインポート依存はありません。Phase 3 は以前のフェーズが生成したファイルを読み込みますが、コードを共有しません。
...
**Location**: `reorg/output/polyhedra/<class>/<name>/exact.jsonl`
**Format**: JSON Lines (one record per line)
**Purpose**: Verified partial unfoldings with overlap classification

**生成元**: Phase 3（`exact`）
**場所**: `reorg/output/polyhedra/<class>/<name>/exact.jsonl`
...
- `--poly CLASS/NAME`: Polyhedron identifier (e.g., `archimedean/s07`, `johnson/n20`) **[required]**
...
- **polyhedron.json**: Must exist in `reorg/data/polyhedra/<class>/<name>/`

### Typical Workflow / 典型的なワークフロー

```bash
# Step 1: Generate raw.jsonl (Phase 1)
PYTHONPATH=reorg/python python -m rotational_unfolding run --poly johnson/n66

# Step 2: Remove isomorphic duplicates (Phase 2)
PYTHONPATH=reorg/python python -m nonisomorphic run --poly johnson/n66
...
- **Phase 1 specification**: `reorg/docs/PHASE1_RUN.md` (upstream data contract)
- **Phase 2 specification**: `reorg/docs/PHASE2_NONISO.md` (input contract)
- **Phase 3 implementation**: `reorg/python/exact/`
...
- **Phase 1 仕様**: `reorg/docs/PHASE1_RUN.md`（上流データ契約）
- **Phase 2 仕様**: `reorg/docs/PHASE2_NONISO.md`（入力契約）
- **Phase 3 実装**: `reorg/python/exact/`
