...
All commands require `PYTHONPATH=python` and are run from the repository root.

すべてのコマンドは `PYTHONPATH=python` を必要とし、リポジトリルートから実行します。

The `--poly` argument takes a path to a polyhedron data directory (e.g., `data/polyhedra/archimedean/s07`).
This path supports shell Tab completion.

`--poly` 引数は多面体データディレクトリへのパスを取ります（例: `data/polyhedra/archimedean/s07`）。
...
All outputs are written to `output/polyhedra/<class>/<name>/`:
...
- C++ binary (`cpp/rotunfold`) must be built — required by Phase 1
- Polyhedron data in `data/polyhedra/`
